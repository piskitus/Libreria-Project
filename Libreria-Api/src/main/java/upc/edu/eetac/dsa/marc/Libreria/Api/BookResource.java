...
import upc.edu.eetac.dsa.marc.Libreria.Api.model.Author;
import upc.edu.eetac.dsa.marc.Libreria.Api.model.Books;
import upc.edu.eetac.dsa.marc.Libreria.Api.model.BooksCollection;
...
public class BookResource {
	...
	@GET
	@Produces(MediaType.LIBRERIA_API_BOOKS_COLLECTION)
	public BooksCollection getBooks(@QueryParam("length") int length,
		...
		try {
			...
			while (rs.next()) {
				...
				book.setAuthors(rs.getString("author"));//hay que enviar la lista de autores en vez de un autor
	...
	@GET
	@Path("/{title}")
	@Produces(MediaType.LIBRERIA_API_BOOKS)
	public Response getBook(@PathParam("title") String title,
		...
		Books book = getBooksFromDatabase(title);
		...
		String eTagDigest = DigestUtils.md5Hex(book.getTitle()
				+ book.getAuthors() + book.getEdition() + book.getEditonDate()
	...
	@POST
	@Consumes(MediaType.LIBRERIA_API_BOOKS)
	@Produces(MediaType.LIBRERIA_API_BOOKS)
	public Books createBook(Books book) {
		...
		try {
			...
			stmt.setString(2, book.getAuthors());// tiene q ser una lista
			...
			if (rs.next()) {
				...
				book = getBooksFromDatabase(title);
			}
		} catch (SQLException e) {
	...
	@Consumes(MediaType.LIBRERIA_API_BOOKS)
	@Produces(MediaType.LIBRERIA_API_BOOKS)
	public Books updateBook(@PathParam("title") String title, Books book) {
		// book.setEditonDate("1/13"); // Viene como "null" desde Postman, no sé
		// por qué
		// validateBook(book);
	...
	private Books getBooksFromDatabase(String title) {
		Books book = new Books();
		...
		try {
			...
			ResultSet rs = stmt.executeQuery();
			if (rs.next()) {
				book.setAuthors(rs.getString("author"));//tiene q ser una lista
				book.setEdition(rs.getString("edition"));
				book.setEditonDate(rs.getString("editionDate"));
				book.setLanguage(rs.getString("language"));
				book.setPrintingDate(rs.getString("printingDate"));
